...
def main():
    ...
    def total_time_waiting(profile, times):
        return sum(times)

    total_time = sum(profile.request_time for profile in profiles)
    aggregated = aggregate_profiles(profiles, total_time_waiting)

    print "Waited a total of %d ms" % total_time

    for klass, results in aggregated.items():
        sum_time = sum(results)

        print "Waited %d ms for %s (%f%% of total time)" % (sum_time, klass, \
                                                                float(sum_time) / total_time * 100.0)
